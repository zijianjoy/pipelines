...
import { ExecutionFlowElementData, FlowElementDataBase } from './Constants';
...
function getIcon(state: Execution.State | undefined) {
  if (state === undefined) {
    ...
    return (
      <div className='px-2 h-full flex flex-col justify-center rounded-r-lg bg-gray-100'>
        <ListAltIcon className='text-gray-500' />
      </div>
    );
  }
  switch (state) {
    case Execution.State.UNKNOWN:
      return (
        <div className='px-2 h-full flex flex-col justify-center rounded-r-lg bg-gray-300'>
          <RemoveCircleOutlineIcon className='text-black' />
        </div>
      );
    case Execution.State.NEW:
      return (
        <div className='px-2 h-full flex flex-col justify-center rounded-r-lg bg-yellow-100'>
          <PowerSettingsNewIcon className='text-yellow-500' />
        </div>
      );
    ...
    case Execution.State.FAILED:
      return (
        <div className='px-2 h-full flex flex-col justify-center rounded-r-lg bg-red-100'>
          <ErrorIcon className='text-red-500' />
        </div>
      );
    case Execution.State.CANCELED:
      return (
        <div className='px-2 h-full flex flex-col justify-center rounded-r-lg bg-blue-100'>
          <SyncDisabledIcon className='text-blue-500' />
        </div>
      );
